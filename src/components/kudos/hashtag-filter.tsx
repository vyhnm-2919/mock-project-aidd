...
const FALLBACK_HASHTAGS = [
  "Dedicated", "Inspiring", "Creative", "TeamPlayer", "ProblemSolver",
  "Leadership", "Innovation", "Mentoring", "GoingExtraMile", "RootFurther",
];
...
export function HashtagFilter({ label, selected, onSelect }: HashtagFilterProps): React.ReactElement {
  const [open, setOpen] = useState(false);
  const [options, setOptions] = useState<string[]>(FALLBACK_HASHTAGS);
        ...
        >
          ...
          {options.map((opt) => (
            <button
              ...
              className={`w-full text-center p-4 rounded font-montserrat text-base font-bold tracking-[0.5px] cursor-pointer transition-colors ${selected === opt ? "bg-[rgba(255,234,158,0.10)] text-white [text-shadow:0_4px_4px_rgba(0,0,0,0.25),0_0_6px_#FAE287]" : "text-white hover:bg-[rgba(255,234,158,0.10)]"}`}
            >
              #{opt}
            </button>
          ))}
